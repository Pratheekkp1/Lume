...
export default function VideoProjects() {
  ...
  async function fetchProjects() {
    const { data, error } = await supabase
      .from('video_projects')
      .select('*, video_clips!project_id(id)')
      .order('created_at', { ascending: false })
    if (!error && data) {
      const coverIds = data.filter(p => p.cover_clip_id).map(p => p.cover_clip_id)
      let coverMap = {}
      if (coverIds.length > 0) {
        const { data: coverClips } = await supabase
          .from('video_clips')
          .select('id, file_path')
          .in('id', coverIds)
        if (coverClips) {
          coverClips.forEach(c => {
            coverMap[c.id] = supabase.storage.from('Videos').getPublicUrl(c.file_path).data.publicUrl
          })
        }
      }
      setProjects(data.map(p => ({
        ...p,
        cover_url: p.cover_clip_id ? coverMap[p.cover_clip_id] || null : null,
      })))
    }
    setLoading(false)
  }
  ...
  async function deleteProject() {
    ...
    setDeleting(true)
    await supabase.from('video_clips').delete().eq('project_id', editTarget.id)
    const { error } = await supabase.from('video_projects').delete().eq('id', editTarget.id)
    if (!error) {
      setEditTarget(null)
      fetchProjects()
    }
    ...
    <div className="p-7">
      ...
      <div className="flex items-end justify-between mb-5">
        <div>
          ...
          <h1 className="font-serif text-3xl text-stone-800">Video Projects</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-center">
          ...
          <p className="text-stone-400 text-xs">Create your first video project to get started</p>
        </div>
      ) : (
              ...
              <div className="flex flex-col gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Wedding Film — Mia & Tom"
