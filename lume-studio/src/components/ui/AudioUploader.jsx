...
// item.status: 'uploading' | 'done' | 'error' | 'duplicate' | 'skipped'

export default function AudioUploader({ projectId, onUploadComplete }) {
  ...
  const onDrop = useCallback(
    async (acceptedFiles) => {
      ...
      // Check which filenames already exist in this project
      ...
      // Upload non-duplicates in parallel
      await Promise.all(
      ...
      // If no duplicates need resolving, we're done
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    accept: {
      "audio/*": [".mp3", ".wav", ".aiff", ".aif", ".flac", ".m4a", ".aac", ".ogg", ".opus"],
    },
  ...
  return (
    <div className="p-6">
      ...
      >
        ...
        {isDragActive ? (
          <p className="text-sm text-stone-600">Drop tracks here...</p>
        ) : (
          <>
            <p className="text-sm font-medium text-stone-600 mb-1">Drag & drop audio tracks here</p>
            <p className="text-xs text-stone-400">or click to browse · MP3, WAV, AIFF, FLAC, M4A, AAC, OGG</p>
          </>
        )}
      </div>
